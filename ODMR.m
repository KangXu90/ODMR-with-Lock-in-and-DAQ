...
% % ni.addAILine('Dev1/ai0',0);
% % handles.d = ni;
%  addAnalogInputChannel(handles.d,'Dev1', 'ai0', 'Voltage');
...
% handles.s = RohdeSchwarzSignalGenerator('tcpip','134.74.27.145',5025);
% 
% fopen(handles.s);
...
%  fprintf(handles.s, ':SOUR:FREQ:MODE MAN');  % Setting the signal generator to sweep mode
...
for m=1:M
    fprintf(handles.s,':SOUR:SWE:RES:ALL');  % Reset Sweep from start frequency
%     fprintf(handles.lock,['auxv1,','10']);
...
%              fprintf(handles.s, ':SOUR:FREQ: MAN UP');  % Setting the signal generator to sweep mode

            %            pause(2);
         data=handles.d.startForeground; % data acquistion with NIDAQ
%             
%             % ????
%             time = (0:length(data)-1) / handles.d.Rate;
%             figure;
%             plot(time, data);
%             xlabel('Time (s)');
%             ylabel('Voltage');
%             title('Analog Input Data');
%             grid on;
%             
            % ??FFT
            fft_data = fft(data);
            L = length(data);
            frequencies = handles.d.Rate * (0:(L/2)) / L;
            start_frequency = 0.02;  % Start frequency in Hz
            end_frequency = 1;    % End frequency in Hz
            
            % Find the indices corresponding to the defined frequency range
            start_index = find(frequencies >= start_frequency, 1);
            end_index = find(frequencies <= end_frequency, 1, 'last');
            max_value = max(abs(fft_data(start_index:end_index)));
            
            data_m(m,n)= max_value;
...
%mkdir('ODMRdata')
save(['D:\Data\Kang\charge injection\cw-PDMR\ODMR contrast'...
